...
public class HandVisual : MonoBehaviour
{
    ...
    GameObject CreateACardAtPosition(CardAsset c, Vector3 position, Vector3 eulerAngles)
    {
        ...
        else
        {
            ...
            if (c.Targets == TargetingOptions.NoTarget)
            {
                ...
                card.SetActive(true);
                card.GetComponent<OneCardManager>().CardFaceGlowImage.enabled = false;
            }
            else
            {
                ...
                Debug.Log("TARGETS: " + dragSpell.Targets);
                Debug.Log("TARGETS 2: " + card.GetComponent<OneCardManager>().Targets);
                card.SetActive(true);
                card.GetComponent<OneCardManager>().CardFaceGlowImage.enabled = false;
            }
    ...
    public void GivePlayerACard(CardAsset c, int UniqueID, bool fast = false, bool fromDeck = true)
    {
        ...
        Debug.Log("Return from CreateCardAtPosition");
       ...
       // Debug.Log("BEFORE ADD CARD");
        AddCard(card);
        //Debug.Log("Return from AddCard");

        // Bring card to front while it travels from draw spot to hand
        //Debug.Log("BEFORE WHEREISCREATURE");
        WhereIsTheCardOrCreature w = card.GetComponent<WhereIsTheCardOrCreature>();
        w.BringToFront();
        w.Slot = 0;
        w.VisualState = VisualStates.Transition;
        //Debug.Log("After WhereisCreature");
       ...
       // Debug.Log("Before Change Status");
        ...
        //- Not working to delay code OK PArea.PDeck.NumOfCardsInDeck.text = deck.cards.Count.ToString();
    ...
    public void PlayASpecialFromHand(int CardID)
    {
        GameObject card = IDHolder.GetGameObjectWithID(CardID);
        PlayASpecialFromHand(card);
    }

    public void PlayASpecialFromHand(GameObject CardVisual)
    {
        Command.CommandExecutionComplete();
