...
public class CardCollection : MonoBehaviour 
{
    ...
    void Awake()
    {
        ...
        Debug.Log("In Awake CardCollection");
        Debug.Log("Number of allCardsArray: " + allCardsArray.Length);
                ...
                CharacterAsset asset = null, string keyword = "", int manaCost = -1, bool includeTokenCards = false)
    {
        ...
        string cardtype = "X";
        //if (manaCost == 0)
        //    cards = cards.Where(card => card.AttackDefense == def);

        switch(manaCost)
        {
            case 0: //Attacks
                cardtype = "A";
                cards = cards.Where(card => card.AttackDefense == cardtype);
                break;
            case 1: //Blocks & Guards
                cardtype = "D";
                cards = cards.Where(card => card.AttackDefense == cardtype || card.AttackDefense == "G");
                break;
            case 2: //Dodges
                cardtype = "D";
                cards = cards.Where(card => card.AttackDefense == cardtype && card.TypeOfAttack == AttackTypes.Dodge);
                break;
            case 3: //Special Attacks
                cardtype = "A";
                cards = cards.Where(card => card.AttackDefense == cardtype && card.TypeOfAttack == AttackTypes.Special);
                break;
            case 4:
                cardtype = "S";
                cards = cards.Where(card => card.AttackDefense == cardtype && card.TypeOfSpecial == SpecialType.Event);
                break;
            case 5:
                cardtype = "S";
                cards = cards.Where(card => card.AttackDefense == cardtype && card.TypeOfSpecial == SpecialType.Situation);
                break;
            case 6:
                cardtype = "S";
                cards = cards.Where(card => card.AttackDefense == cardtype && card.TypeOfSpecial == SpecialType.Location);
                break;
            case 7:
                cardtype = "S";
                cards = cards.Where(card => card.AttackDefense == cardtype && card.TypeOfSpecial == SpecialType.Object);
                break;
            case 8:
                cardtype = "S";
                cards = cards.Where(card => card.AttackDefense == cardtype && card.TypeOfSpecial == SpecialType.Edge);
                break;
